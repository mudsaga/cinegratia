...
// Função de segurança que barra qualquer tentativa de alteração no modo visitante
function verificarPermissaoEdicao() {
    if (!loggedInUser) {
        alert("Modo de visualização: você precisa entrar com uma conta e selecionar um perfil para fazer alterações.");
        return false;
    }
    return true;
}
    ...
    membersData.forEach(m => {
        ...
        btn.addEventListener('click', () => {
            if (isEditingProfiles) {
                ...
                const navProfileEl = document.getElementById('nav-profile-switch');
                if (navProfileEl) {
                    navProfileEl.style.display = 'flex';
...
// Variáveis da barra de navegação no topo declaradas apenas uma vez
...
// Inicializa a aplicação diretamente exibindo o catálogo no modo visualizador livre
renderApp();

// Verifica o estado do login guardado no localStorage ao carregar o script
const estaLogadoNoSistema = localStorage.getItem('gratia_logado') === 'true';

// Declara referências locais exclusivas para evitar erros com constantes globais
const telaDeLoginEl = document.getElementById('login-screen');
const botaoEntrarTopoEl = document.getElementById('btn-open-login');
const botaoTrocarPerfilEl = document.getElementById('nav-profile-switch');

if (estaLogadoNoSistema) {
    // Se estiver logado, abre direto na tela de escolha de perfis
    if (telaDeLoginEl) {
        telaDeLoginEl.style.display = 'flex';
        renderLoginScreen();
    }
    if (botaoEntrarTopoEl) botaoEntrarTopoEl.style.display = 'none';
    if (botaoTrocarPerfilEl) botaoTrocarPerfilEl.style.display = 'none';
} else {
    // Se não estiver logado, abre no modo visitante livre
    if (telaDeLoginEl) telaDeLoginEl.style.display = 'none';
    if (botaoEntrarTopoEl) {
        botaoEntrarTopoEl.style.display = 'block';
        botaoEntrarTopoEl.onclick = () => {
